...
<html lang="en">
    ...
    <div class="row">
	    <form class="form-horizontal" id="fn-content-set-form" role="form" method="post" action="${path }/report/setContent">
  			  ...
  			  success: function(data){
  				  if(data!=null && data.aaData !=null &&data.aaData.length!=0){
  					  ...
  					  $("#choose-content").children().remove();
   					  $.ajax({
						  type: 'POST',
						  url: contextPath + '/report/getContentsByInfo',
						  data: {"id":id},
		  ...
		  dataType: 'json'
   		 ...
   		 $("#fn-btn-confirm").click(function(){
   			 if($("#choose-content").val()!=null && $("#choose-content").val()!=""){
   				...
   				$.ajax({
					  type: 'POST',
					  url: contextPath + '/report/setContent?'+serial,
					  data: {"id":id},
					  success: function(data){
						  if(data == "success"){
							  parent.location.href="${path}/view/report/list.jsp?msg=操作成功！";
							  $("#fn-btn-cancle").click();
						  } else {
							  $.alert(data);
						  }
   		 ...
   		 function sort(){
   			 ...
   			 $(".seq").each(function(){
   				 ...
   				 $(this).parent().find(".seq-input").attr("name","reportInfoDetails["+(i-1)+"].seq");
   				 $(this).parent().find(".content-id-input").attr("name","reportInfoDetails["+(i-1)+"].reportDetail.id");
   				 i++;
   			 });
   		 }
    });
